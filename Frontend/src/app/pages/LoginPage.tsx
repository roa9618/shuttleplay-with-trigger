        ...
        <div className = "bg-card/80 backdrop-blur-sm border-2 border-border rounded-3xl p-10 shadow-2xl shadow-primary/10">
          <form onSubmit = {handleSubmit} className = "space-y-6">
            <div className = "space-y-2">
              ...
              <Input id = "email" type = "email" placeholder = "이메일을 입력하세요" className = "rounded-xl h-12" value = {formData.email} onChange = {(e) => setFormData({ ...formData, email: e.target.value })}
                required
              />
            </div>
            ...
            <div className = "space-y-2">
              ...
              <Input id = "password" type = "password" placeholder = "비밀번호를 입력하세요" className = "rounded-xl h-12" value = {formData.password} onChange = {(e) => setFormData({ ...formData, password: e.target.value })}
                required
              />
            </div>

            <Link to = "/password-reset" className = "block text-sm text-muted-foreground hover:text-primary transition-colors text-right w-full"
            >
              비밀번호 찾기
            </Link>
          ...
          <div className = "space-y-3">
            {socialProviders.map((provider) => (
              <Button key = {provider.name} type = "button" variant = "outline" className = {`w-full rounded-full h-12 justify-start gap-3 border-2 ${provider.className}`} onClick = {() => showMessage(`${provider.name} 로그인 연결을 준비했습니다.`)}
              >
